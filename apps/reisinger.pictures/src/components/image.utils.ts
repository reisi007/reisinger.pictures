...
export async function getMetadataForImageSlug(
  ...
  if (!data || data.description === "" || data.description === undefined) {
    const message = `Image "${imageSlug}" has no alt text. Please add a description to the image metadata in the content folder.`;
